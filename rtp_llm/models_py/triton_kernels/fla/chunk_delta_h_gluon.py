...
def chunk_gated_delta_rule_fwd_h_gluon(
    ...
    Args:
        state_dtype: dtype of the per-chunk hidden state buffer ``h``.
            If ``None``, this Gluon path falls back to ``initial_state.dtype``
            (or ``k.dtype`` when ``initial_state is None``) as an internal
            CDNA4 optimization — the Gluon kernel branches on ``H_FP32`` to
            choose between fp32 and bf16 buffers, and a narrower buffer wins
            ~10-18% on MI355X for TP2/H≤32/T≤64K. Callers that need a
            specific layout-stable dtype across backends should pass it
            explicitly.
    """
    ...
    if state_dtype is None:
        # Internal CDNA4 optimization: pick state dtype from initial_state
        # (or k) to enable the bf16 fast path in the Gluon kernel.
