...
def chunk_gated_delta_rule_fwd_h(
    ...
    if _gluon_available:
        ...
        K = k.shape[-1]
        V = u.shape[-1]
        # NOTE: Gluon CDNA4 path is temporarily disabled. After migrating the
        # Triton fallback + chunk_o + fused_recurrent to V-first cache layout
        # (b_h shape (BV, BK), byte formula v*K + k) to match SGL main and the
        # on-disk ssm_states declaration, the Gluon kernel still writes h with
        # the legacy K-first formula (b_h shape (64, BV), byte k*V + v). With
        # K == V == 128 the byte addresses coincide but the per-element
        # semantics are transposed, so downstream chunk_o reading via the new
        # V-first view would pick up h[K=row, V=col] when it expects
        # h[V=row, K=col] and silently corrupt o. Re-enable after porting the
        # Gluon mfma layout to V-first and re-validating on MI355X.
