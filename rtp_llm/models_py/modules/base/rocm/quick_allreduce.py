...
def _parse_bool_env(name: str, default: bool) -> bool:
    ...
    if s in _BOOL_FALSE:
        return False
    logger.warning(
        "Quick AllReduce: invalid %s=%r, using default=%s", name, raw, default,
    )
    return default
...
class _QuickARManager:
    ...
    def initialize(self, group: ProcessGroup, device_id: int) -> None:
            ...
            if self.world_size == 1 or self.world_size not in _SUPPORTED_WORLD_SIZES:
                ...
                if not any(gfx in gcn_arch for gfx in supported_archs):
                    ...
                    self.ptr = ops.init_custom_qr(
                        self.rank, self.world_size, max_size_bytes,
                    )
                    self.max_size = (
        ...
        # Phase 1 consensus: every rank must succeed for the IPC-handle
        # exchange below to be safe. If any rank failed, all ranks abort.
        try:
            world_size = dist.get_world_size(group=group)
            ok_flags = [None] * world_size
            dist.all_gather_object(ok_flags, local_ok, group=group)
        except Exception as exc:
            logger.warning("Quick AllReduce init consensus failed: %s", exc)
            ok_flags = [False] * (self.world_size or 1)
            local_ok = False

        if not all(bool(x) for x in ok_flags):
            if local_ok and self.ptr != 0:
                # Local success but a peer failed — release our resources.
                try:
                    import aiter as ops
                ...
                self.ptr = 0
            if local_err:
                logger.info("Quick AllReduce disabled: %s", local_err)
            else:
                logger.info("Quick AllReduce disabled: a peer rank failed init")
            self.disabled = True
            self.initialized = True
            return

        # Phase 2: all ranks succeeded — exchange IPC handles and barrier.
        try:
            import aiter as ops
            ...
            logger.info(
                ...
                device_id, self.rank, self.world_size,
                quant_str, self.use_fp16_kernels, self.max_size // _MB,
            )
        except Exception as exc:
